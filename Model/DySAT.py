...
class DySAT(nn.Module):
    ...
    def forward(self, graphs, gate):
        ...
        '''
        GCN_emb_list = [torch.Tensor(self.args['nodes_info'][-1], self.structural_layer_config[-1])[:,None,:].to(self.device) for t in range(self.args['timesteps'])]
        RNN_emb_list = [torch.Tensor(self.args['nodes_info'][-1], self.temporal_layer_config[-1])[:,None,:].to(self.device) for t in range(self.args['timesteps'])]
