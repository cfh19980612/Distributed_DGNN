...
def Cross_edges(timesteps, adjs, nodes_list, current_workload, workload, flag):
    ...
    if flag == 0:
        ...
        nodes_idx_list = [edge_target[idx] for idx in idx_list]
        # print(nodes_idx_list)
        nodes_idx_list_new = filter(lambda val: val !=  torch.tensor([], dtype=torch.int64), nodes_idx_list)
        print(nodes_idx_list_new)
        nodes_idx = torch.cat((nodes_idx_list_new), dim=0)
